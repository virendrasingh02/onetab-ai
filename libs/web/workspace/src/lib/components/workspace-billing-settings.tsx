...
export function WorkspaceBillingSettings({
  ...
  const [selectedPlanForDowngrade, setSelectedPlanForDowngrade] = useState<PlanTier | null>(null);
  ...
  const { data: downgradeImpact, isLoading: isCheckingDowngrade } = useQuery<DowngradeImpactSummary>({
    queryKey: queryKeys.billing.downgradeImpact(workspaceId ?? '', selectedPlanForDowngrade ?? ''),
    queryFn: () => billingApi.downgradeImpact(workspaceId as string, selectedPlanForDowngrade as PlanTier),
    enabled: !!workspaceId && !!selectedPlanForDowngrade,
  });

  // Upgrade Mutation
  const upgradeMutation = useMutation<WorkspaceBillingSummary, Error, UpgradePlanInput>({
    mutationFn: (input: UpgradePlanInput) => billingApi.upgrade(workspaceId as string, input),
    onSuccess: (updated) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.billing.all(workspaceId as string) });
  ...
  // Downgrade Mutation
  const downgradeMutation = useMutation<WorkspaceBillingSummary, Error, PlanTier>({
    mutationFn: (targetPlan: PlanTier) =>
      billingApi.downgrade(workspaceId as string, { targetPlan, acknowledgeOverages: true }),
    onSuccess: (updated) => {
      queryClient.invalidateQueries({ queryKey: queryKeys.billing.all(workspaceId as string) });
      queryClient.invalidateQueries({ queryKey: queryKeys.workspaces.all() });
  ...
  >({
    ...
    onSuccess: (res) => {
      ...
      toast.success(res.message || 'Our enterprise team will reach out within 1 business day.');
    },
    onError: (err) => {
  ...
  const currentWorkspaceName = propWorkspaceName || workspace?.name || 'Workspace';

  return (
    <div className="space-y-10">
      {/* 1. Header & Active Plan Overview */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 rounded-2xl border border-border bg-gradient-to-br from-card via-card to-surface-muted/40 shadow-xs">
        <div className="space-y-1.5">
          <div className="flex items-center gap-3">
            <h2 className="text-xl font-bold tracking-tight text-foreground">Plans & Billing</h2>
          ...
          <p className="text-xs md:text-sm text-muted-foreground">
            Manage your subscription tier, resource quotas, enterprise custom LLM, and payment invoices for{' '}
            <strong className="text-foreground">{currentWorkspaceName}</strong>.
          </p>
        </div>

        {subscription?.renewAt && plan !== 'starter' && (
          <div className="flex items-center gap-2 text-xs bg-surface-muted px-3 py-1.5 rounded-lg border border-border text-muted-foreground shrink-0">
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-base font-bold text-foreground">Available Upgrade Plans</h3>
            <p className="text-xs text-muted-foreground">
              Scale your team with flexible monthly or discounted annual subscriptions.
            </p>
          </div>

          {/* Monthly / Annual Toggle Switch */}
          <div className="flex items-center gap-3 bg-surface-muted/80 p-1.5 rounded-xl border border-border self-start sm:self-auto text-xs">
            <span className={`font-medium ${!isAnnual ? 'text-foreground' : 'text-muted-foreground'}`}>
              Monthly
            </span>
            <Switch checked={isAnnual} onCheckedChange={setIsAnnual} />
            <span className={`font-medium flex items-center gap-1.5 ${isAnnual ? 'text-foreground font-semibold' : 'text-muted-foreground'}`}>
              Annual Billing
              <Badge variant="primary" className="text-[10px] px-1.5 py-0 font-bold bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/20">
                SAVE 20%
              </Badge>
            </span>
          </div>
          ...
          {PLAN_TIERS.map((tier) => {
            ...
            return (
              ...
              >
                ...
                {def.badgeText ? (
                  <div
                    className={`absolute top-0 right-0 px-3 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-bl-lg ${
                      isCurrent
                        ? 'bg-primary text-primary-foreground'
                        : isEnterprise
                          ? 'bg-amber-500 text-white'
                          : 'bg-primary/20 text-primary border-b border-l border-primary/30'
                    }`}
                  >
                    {isCurrent ? 'CURRENT PLAN' : def.badgeText}
                  </div>
                ) : isCurrent ? (
                  <div className="absolute top-0 right-0 px-3 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-bl-lg bg-primary text-primary-foreground">
                    CURRENT PLAN
                  </div>
                ) : null}
                  ...
                  <div className="pt-3">
                    {isEnterprise ? (
                      <div className="flex items-baseline gap-1">
                        <span className="text-2xl font-black tracking-tight text-foreground">Custom</span>
                    ...
                    {!isEnterprise && isAnnual && def.pricing.annualDiscountPercent > 0 ? (
      ...
      {plan !== 'starter' && invoices.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-foreground">Billing History & Receipts</h3>
          </div>
              ...
              <TableBody>
                {invoices.map((inv: InvoiceItemDto) => (
                  <TableRow key={inv.id}>
                    ...
                    <TableCell className="text-xs text-foreground">{inv.description}</TableCell>
                    <TableCell className="text-xs font-semibold text-foreground">
                      ${(inv.amountCents / 100).toFixed(2)}
                    </TableCell>
                    <TableCell>
                      <Badge variant="primary" className="text-[10px] font-bold bg-emerald-500/15 text-emerald-600 border-emerald-500/20">
                        {inv.status}
                      </Badge>
                    </TableCell>
                  </TableRow>
            ...
            <div className="space-y-5">
              <DialogHeader className="text-left space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <PlanBadge plan={selectedPlanForUpgrade} size="sm" variant="gradient" />
                </div>
                <DialogDescription className="text-xs text-muted-foreground">
                  Instantly unlock advanced capabilities and expand limits for {currentWorkspaceName}.
                </DialogDescription>
              </DialogHeader>

              {/* Seats selector */}
              <div className="space-y-2 p-3.5 rounded-xl border border-border bg-surface-muted/50 text-xs">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-foreground">Team Member Seats</span>
                  <div className="flex items-center gap-2">
                    <Button
                      variant="outline"
                      size="icon"
                ...
                <p className="text-[11px] text-muted-foreground">
                  You can adjust your seat allocation anytime as your team grows.
                </p>
              </div>
              ...
              <div className="p-3.5 rounded-xl border border-primary/20 bg-primary/5 space-y-1.5 text-xs">
                  ...
                  <span>
                    ...
                    {(
                      ...
                      (isAnnual
                        ...
                        : PLANS_CONFIG[selectedPlanForUpgrade].pricing.monthly) *
                      (isAnnual ? 12 : 1)
                    ).toLocaleString()}
                    {isAnnual ? '/year' : '/month'}
                  </span>
                ...
                <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                  ...
                  <span className="capitalize">{isAnnual ? 'Annual (20% Savings)' : 'Monthly'}</span>
                </div>
              </div>
          ...
          {selectedPlanForDowngrade && (
            <div className="space-y-4">
              <DialogHeader className="text-left space-y-1">
                ...
                <DialogDescription className="text-xs text-muted-foreground">
                  Please review the impact on your resources and restricted features before confirming.
                </DialogDescription>
              </DialogHeader>

              {isCheckingDowngrade ? (
                <div className="py-6 flex justify-center text-xs text-muted-foreground">
                  <RefreshCw className="size-4 animate-spin mr-2" /> Auditing resources...
                </div>
              ) : (
                <div className="space-y-3 text-xs">
                  {downgradeImpact?.warnings && downgradeImpact.warnings.length > 0 ? (
                    <div className="p-3.5 rounded-xl border border-amber-500/30 bg-amber-500/10 space-y-2 text-amber-900 dark:text-amber-200">
                    ...
                    <div className="p-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-800 dark:text-emerald-200 text-xs">
                      ✓ Current usage fits within {PLANS_CONFIG[selectedPlanForDowngrade].name} limits.
                    </div>
                  )}

                  <div className="p-3.5 rounded-xl border border-border bg-surface-muted/50 space-y-1.5">
                    <p className="font-semibold text-foreground">Safe Retention Guarantee:</p>
                    <p className="text-[11px] text-muted-foreground leading-relaxed">
                      Your existing projects, members, and uploaded files will <strong>not</strong> be deleted. However, creating new resources beyond the lower tier limits will be restricted.
                    </p>
                  </div>
                </div>
              )}
              ...
              <DialogFooter className="pt-2 flex flex-row items-center justify-end gap-2">
                ...
                <Button
                  variant="destructive"
                  size="sm"
                  onClick={() => downgradeMutation.mutate(selectedPlanForDowngrade)}
        ...
        <DialogContent className="sm:max-w-lg p-6">
          <div className="space-y-4">
            <DialogHeader className="text-left space-y-1">
              ...
              <DialogDescription className="text-xs text-muted-foreground">
                Get a custom quote with private LLM integration, dedicated infrastructure, and enterprise SLA.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <label className="font-semibold text-foreground">Team Size</label>
                  <Input
              ...
              <div className="space-y-1">
                <label className="font-semibold text-foreground">Custom LLM & Private Model Needs (Optional)</label>
                <Input
                  value={enterpriseForm.customLlmRequirements}
                  onChange={(e) => setEnterpriseForm({ ...enterpriseForm, customLlmRequirements: e.target.value })}
